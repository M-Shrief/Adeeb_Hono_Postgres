...
poemRoute.get('/', async (c) => {
  ...
  if (!poems) return c.json({message: ERROR_MSG.NOT_AVAILABLE}, HttpStatusCode.NOT_FOUND);
...
poemRoute.get('/:id', idParamValidator(), async (c) => {
  ...
  if(!poem) return c.json({message: ERROR_MSG.NOT_FOUND}, HttpStatusCode.NOT_FOUND)
...
poemRoute.post('/', jsonValidator(createSchema), async (c) => {
  ...
  if (!newPoem) c.json({message: ERROR_MSG.NOT_VALID}, HttpStatusCode.NOT_ACCEPTABLE);
...
poemRoute.post('/many', jsonValidator(array(createSchema)), async (c) => {
  ...
  if (!newPoems) c.json({message: ERROR_MSG.NOT_VALID}, HttpStatusCode.NOT_ACCEPTABLE);
...
poemRoute.delete('/:id', idParamValidator(), async (c) => {
  ...
  if (!deletedPoem) c.json({message: ERROR_MSG.NOT_FOUND}, HttpStatusCode.NOT_ACCEPTABLE);
