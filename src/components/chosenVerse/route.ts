...
chosenVerseRoute.get('/', async (c) => {
  ...
  if (!chosenVerses) return c.json({message: ERROR_MSG.NOT_AVAILABLE}, HttpStatusCode.NOT_FOUND);
  return c.json(chosenVerses, HttpStatusCode.OK);
});

chosenVerseRoute.get('/random', async (c) => {
  const num = Number(c.req.query('num'))
  if(isNaN(num)) return c.json({message: ERROR_MSG.NOT_VALID}, HttpStatusCode.NOT_ACCEPTABLE)
...
chosenVerseRoute.get('/:id', idParamValidator(), async (c) => {
  ...
  if(!chosenVerse) return c.json({message: ERROR_MSG.NOT_FOUND}, HttpStatusCode.NOT_FOUND)
...
chosenVerseRoute.post('/', jsonValidator(createSchema), async (c) => {
  ...
  if (!newChosenVerse) c.json({message: ERROR_MSG.NOT_VALID}, HttpStatusCode.NOT_ACCEPTABLE);
...
chosenVerseRoute.post('/many', jsonValidator(array(createSchema)), async (c) => {
  ...
  if (!newChosenVerses) c.json({message: ERROR_MSG.NOT_VALID}, HttpStatusCode.NOT_ACCEPTABLE);
...
chosenVerseRoute.delete('/:id', idParamValidator(), async (c) => {
  ...
  if (!deletedChosenVerse) c.json({message: ERROR_MSG.NOT_FOUND}, HttpStatusCode.NOT_ACCEPTABLE);
