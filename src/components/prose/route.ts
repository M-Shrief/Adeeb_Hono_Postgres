...
proseRoute.get('/', async (c) => {
  ...
  if (!proses) return c.json({message: ERROR_MSG.NOT_AVAILABLE}, HttpStatusCode.NOT_FOUND);
...
proseRoute.get('/:id', idParamValidator(), async (c) => {
  ...
  if(!prose) return c.json({message: ERROR_MSG.NOT_FOUND}, HttpStatusCode.NOT_FOUND)
...
proseRoute.post('/', jsonValidator(createSchema), async (c) => {
  ...
  if (!newProse) c.json({message: ERROR_MSG.NOT_VALID}, HttpStatusCode.NOT_ACCEPTABLE);
...
proseRoute.post('/many', jsonValidator(array(createSchema)), async (c) => {
  ...
  if (!newProses) c.json({message: ERROR_MSG.NOT_VALID}, HttpStatusCode.NOT_ACCEPTABLE);
...
proseRoute.delete('/:id', idParamValidator(), async (c) => {
  ...
  if (!deletedProse) c.json({message: ERROR_MSG.NOT_FOUND}, HttpStatusCode.NOT_ACCEPTABLE);
